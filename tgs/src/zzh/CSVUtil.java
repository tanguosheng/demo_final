...
public class CSVUtil {
     ...
     * CSV文件生成方法
     */
    public static File createCSVFile(Map<Bytes, AtomicInteger> head, List<Map.Entry<Bytes, AtomicInteger>> dataList,
...
//            writeRowHead(head, csvWtriter);
     ...
     * 写一行数据方法=
     */
    private static void writeRow(List<Map.Entry<Bytes, AtomicInteger>> row, BufferedWriter csvWriter,
                                 int topNum) throws IOException {
        if (topNum < 0) {
            topNum = row.size();
        }
        for (int i = 0; i < topNum; i++) {
            Map.Entry<Bytes, AtomicInteger> item = row.get(i);
            StringBuffer sb = new StringBuffer();
            String rowStr = sb.append("\"").append(new String(item.getKey().getKey())).append("\",").append(item.getValue()).toString();
